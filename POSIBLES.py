...
COLUMNAS_ELIMINADOS = ["Cliente", "Residuo", "Tipología", "Línea", "Comercial", "Fecha CC", "CC", "Subtotal"]
...
def generar_resumen(df_posibles):
    if df_posibles is None or df_posibles.empty: messagebox.showinfo("Resumen", "No hay datos en 'POSIBLES'."); return pd.DataFrame({"Mensaje": ["No hay datos"]})
    required_cols = [FECHA_COLUMNA, COMERCIAL_COLUMNA, SUBTOTAL_COLUMNA]; missing_cols = [col for col in required_cols if col not in df_posibles.columns]
    if missing_cols: messagebox.showerror("Error Resumen", f"Faltan columnas: {', '.join(missing_cols)}."); return pd.DataFrame({"Error": [f"Faltan: {', '.join(missing_cols)}"]})
    if not pd.api.types.is_datetime64_any_dtype(df_posibles[FECHA_COLUMNA]): df_posibles[FECHA_COLUMNA] = pd.to_datetime(df_posibles[FECHA_COLUMNA], errors='coerce')
    if not pd.api.types.is_numeric_dtype(df_posibles[SUBTOTAL_COLUMNA]): df_posibles[SUBTOTAL_COLUMNA] = pd.to_numeric(df_posibles[SUBTOTAL_COLUMNA], errors='coerce')
    df_valid = df_posibles.dropna(subset=[FECHA_COLUMNA, SUBTOTAL_COLUMNA]).copy()
    if df_valid.empty: messagebox.showinfo("Resumen", "No hay datos válidos."); return pd.DataFrame({"Mensaje": ["No hay datos válidos"]})
    df_valid = df_valid.copy(); df_valid['Mes'] = df_valid[FECHA_COLUMNA].dt.strftime('%Y-%m')
    df_valid[COMERCIAL_COLUMNA] = df_valid[COMERCIAL_COLUMNA].fillna('Sin Asignar')
    try:
        pivot_resumen = pd.pivot_table(df_valid, values=SUBTOTAL_COLUMNA, index='Mes', columns=COMERCIAL_COLUMNA, aggfunc='sum', fill_value=0, margins=True, margins_name='Total General')
        pivot_resumen = pivot_resumen.rename(index={'Total General': 'Total Comercial'}, columns={'Total General': 'Total Mes'})
        print("Tabla pivote de resumen generada.")
        return pivot_resumen
    except Exception as e: messagebox.showerror("Error Resumen", f"No se pudo generar tabla pivote: {e}"); return pd.DataFrame({"Error": [f"Error pivote: {e}"]})
...
def guardar_cambios_excel_completo(filepath, df_posibles_actualizado, df_nuevos_eliminados):
    ...
    try:
        ...
        df_resumen = generar_resumen(df_posibles_actualizado);
        ...
        with pd.ExcelWriter(filepath, engine='xlsxwriter') as writer:
            for nombre_hoja, df_hoja in hojas_a_escribir.items():
                write_index = (nombre_hoja == SHEET_RESUMEN)
                df_hoja.to_excel(writer, sheet_name=nombre_hoja, index=write_index)
            if SHEET_RESUMEN in hojas_a_escribir:
                workbook = writer.book; worksheet = writer.sheets[SHEET_RESUMEN]
                money_format = workbook.add_format({'num_format': '$ #,##0'})
                start_col = 1; end_col = hojas_a_escribir[SHEET_RESUMEN].shape[1]
                worksheet.set_column(start_col, end_col, None, money_format)
                print(f"Formato moneda aplicado a '{SHEET_RESUMEN}'.")
...
def procesar_y_cargar_csv(csv_filepath, df_principal):
    ...
    try:
        ...
        weight_str = df_csv['weight'].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False).str.strip()
        df_import['Peso CC'] = pd.to_numeric(weight_str, errors='coerce')
    ...
    def mover_seleccionados(self):
        ...
        if guardar_cambios_excel_completo(self.selected_excel_file, df_posibles_actualizado, df_a_mover):
             ...
             if CLIENTE_COLUMNA in self.df_original.columns:
                 clientes_raw = self.df_original[CLIENTE_COLUMNA].astype(str).unique()
                 self.lista_clientes_unicos = sorted([c for c in clientes_raw if c.lower() != 'nan'])
             else: self.lista_clientes_unicos = []
             self.mostrar_todo()
        else:
